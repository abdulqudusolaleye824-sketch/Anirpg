...
await at('bad input and the device-slot route are handled, and the page still serves', async () => {
  ...
  // Push #61 removed the admin token: the open port answers this route
  // directly — the rate limit is the only brake left.
  ...
  assert.ok(!html.includes('id="adminBar"') && !html.includes('ASTRALINK_ADMIN_TOKEN'), 'the owner-ops token UI survived Push #61');
