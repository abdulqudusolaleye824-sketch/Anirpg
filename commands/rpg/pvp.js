...
async function handleVictory(sock, chatId, winner, loser, wId, lId, db, save, wStats, lStats, totalTurns) {
  ...
  const baseNexus=800+Math.floor(loser.level*50)+(winner.pvpStreak>=3?200:0);
  const baseXP=500+(loser.level*30);

  // ── Apply seasonal event bonuses (#3) ──────────────────────
  let finalNexus = baseNexus;
  let finalXP = baseXP;
  let eventBonusMsg = '';
  try {
    if (SeasonManager) {
      const bonused = SeasonManager.applyBonuses({ gold: baseNexus, xp: baseXP });
      finalNexus = bonused.gold ?? baseNexus;
      finalXP   = bonused.xp   ?? baseXP;
      const event = SeasonManager.getActiveEvent();
      if (event && (finalNexus !== baseNexus || finalXP !== baseXP)) {
        eventBonusMsg = `\n${event.emoji} *${event.name} BONUS!* Nexus ×${event.goldMult||1} XP ×${event.xpMult||1}`;
      }
    }
  } catch(e) {}

  const streakBonus=winner.pvpStreak>=5?`\n🔥 *${winner.pvpStreak}-WIN STREAK!* +50 Ne`:winner.pvpStreak>=3?`\n🔥 Win Streak ×${winner.pvpStreak}! +50 Ne`:'';

  // Apply shop buffs
  if (BuffManager) {
    const xpMult = BuffManager.getXpMultiplier(winner);
    const goldMult = BuffManager.getNexusMultiplier(winner);
    finalXP = Math.floor(finalXP * xpMult);
    finalNexus = Math.floor(finalNexus * goldMult);
  }
  winner.gold=(winner.gold||0)+finalNexus;
  ...
  async execute(sock, msg, args, getDatabase, saveDatabase, sender) {
    ...
    if (action==='history'||action==='matches') {
      ...
      history.forEach((m,i) => {
        ...
        txt += `   ELO: ${m.eloChange} | Turns: ${m.turns}${m.goldEarned?` | +${m.goldEarned} 💠`:''}\n`;
        txt += `   _${dateStr}_\n\n`;
      });
      txt += `━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
      return sock.sendMessage(chatId,{text:txt},{quoted:msg});
