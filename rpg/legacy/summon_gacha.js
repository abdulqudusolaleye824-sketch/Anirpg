// PARKED (Push #88p): the multi-banner gacha formerly at commands/rpg/summon.js.
// /summon is now the Summoner class shortcut. This module is kept intact so the
// gacha can be refined and re-wired later under a new command name.
// /summon — Multi-banner gacha system
const BS = require('../utils/BannerSystem');
...
module.exports = {
  ...
  async execute(sock, msg, args, getDatabase, saveDatabase, sender) {
    ...
    if (isTicket) {
      ...
      try{require('../utils/BattlePass').addPassXP(player,'summon_pull',1);}catch(e){}
      try{const _QD=require('../utils/QuestDispatcher');_QD.trackAndNotify(player,'summon',1,sock,sender,chatId);if(item.rarity==='legendary')_QD.trackAndNotify(player,'legendary',1,sock,sender,chatId);}catch(e){}
      saveDatabase();
      const el=BS.ELEMENT_EMOJI[item.element||'none'];
    ...
    try{require('../utils/BattlePass').addPassXP(player,'summon_pull',count);}catch(e){}
    try{require('../../commands/rpg/weekly').trackWeeklyProgress(player,'summon_pull',count);}catch(e){}
    try{const _QD=require('../utils/QuestDispatcher');_QD.trackAndNotify(player,'summon',count,sock,sender,chatId);const _legs=results.filter(r=>r.item.rarity==='legendary').length;if(_legs)_QD.trackAndNotify(player,'legendary',_legs,sock,sender,chatId);}catch(e){}
    saveDatabase();
