...
http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/api/bot-status') {
    try {
      ...
      for (const jid of linkedJids) {
        ...
        // Was this personality activated (via /start or /switch) in any group
        // right now? That is what a dashboard shows as the "active" bot.
        let activeNow = false;
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ bots, total: bots.length, serverTime: Date.now() }));
    } catch (e) {
  ...
  // Returns the most recent QR a not-yet-connected personality is showing,
  // rendered server-side into a PNG data-URI, so the AstraLink web UI can show
  // a scannable image — no terminal, no pairing code, no CDN needed.
  if (req.method === 'GET' && req.url.startsWith('/api/qr')) {
    const u = new URL(req.url, 'http://localhost');
    const personality = (u.searchParams.get('personality') || '').toLowerCase();
    if (!personality) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: 'Missing personality' }));
    }
    const qr = MultiSocketManager.getLatestQr(personality);
    if (!qr) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: 'No active QR for this bot yet. It appears once the bot is waiting for a link.' }));
    }
    let dataUri = null;
    try {
      const QRCode = require('qrcode');
      dataUri = await QRCode.toDataURL(qr, { width: 280, margin: 1 });
    } catch (e) {
      // qrcode package unavailable — the client can still render the raw string.
      console.warn('⚠ qrcode package not installed; returning raw QR string. Run: npm install qrcode');
    }
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ success: true, qr, dataUri }));
  }
